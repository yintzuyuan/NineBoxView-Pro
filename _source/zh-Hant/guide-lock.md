...
- 周圍 8 格顯示對應位置的鎖定字符
- 中央格（位置 4）顯示鎖頭圖示，點擊可切換鎖定功能的啟用狀態

### 與主視窗的關係

鎖定字面板與主視窗共享相同的鎖定字資料，但操作方式不同：

| 項目 | 說明 |
|-----|------|
| 資料同步 | 在任一視窗修改鎖定字，另一視窗即時更新 |
| 操作方式 | 面板以「選取」為核心；主視窗以「⌘ 模式」為核心 |
| 中央格功能 | 面板切換鎖定啟用狀態；主視窗清除所有鎖定 |

> 鎖定字面板的操作設計專注於批量管理，與主視窗的即時鎖定/解鎖操作互補。
...
鎖定字面板的中央格顯示鎖頭圖示，功能與主視窗不同：

- **點擊中央格**：切換鎖定功能的啟用/停用狀態
- 啟用時：鎖頭圖示填滿，背景高亮
- 停用時：鎖頭圖示空心
