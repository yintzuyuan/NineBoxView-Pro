...
| 层级 | 可接收拖曳 | 可输出拖曳 | 说明 |
|------|-----------|-----------|------|
| {{ICON_PEN_NIB}} 编辑字 | ✗ | ✓ | 中央格只能向外输出，无法作为拖放目标 |
| {{ICON_ARTICLE}} 参考字 | ✓（可见层） | ✓ | 可与最上层显示的字符交换位置 |
| {{ICON_LOCK}} 锁定字 | ✓ | ✓ | 可从编辑字或参考字层通过 ⌘ + 拖曳加入 |

> [!TIP]
> 若要将编辑中的字符固定到某个位置，使用 **⌘ + 拖曳**从中央格拖曳到目标位置即可建立锁定字。
...
| {{INFO_MENU_TESTIMONIAL|}} | 分享使用心得至社交媒体 |
