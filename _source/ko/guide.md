...
3층 간의 드래그 조작에는 다른 제한이 있습니다:

| 레이어 | 드래그 수신 가능 | 드래그 출력 가능 | 설명 |
|--------|----------------|----------------|------|
| {{ICON_PEN_NIB}} 편집 글자 | ✗ | ✓ | 중앙 셀은 출력만 가능, 드롭 대상이 될 수 없음 |
| {{ICON_ARTICLE}} 참조 글자 | ✓ (표시 레이어) | ✓ | 최상층에 표시되는 글자와 위치 교환 가능 |
| {{ICON_LOCK}} 잠금 글자 | ✓ | ✓ | 편집 글자 층 또는 참조 글자 층에서 ⌘ + 드래그로 추가 가능 |

> [!TIP]
> 편집 중인 글자를 특정 위치에 고정하려면 **⌘ + 드래그**로 중앙 셀에서 대상 위치로 드래그하여 잠금 글자를 생성할 수 있습니다.
...
| {{INFO_MENU_TESTIMONIAL|}} | 사용 후기를 소셜 미디어에 공유 |
