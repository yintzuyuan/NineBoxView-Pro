...
3層間のドラッグ操作には異なる制限があります：

| レイヤー | ドラッグ受入可 | ドラッグ出力可 | 説明 |
|---------|---------------|---------------|------|
| {{ICON_PEN_NIB}} 編集文字 | ✗ | ✓ | 中央セルは出力のみ可能、ドロップターゲットにはなれない |
| {{ICON_ARTICLE}} 参照文字 | ✓（表示層） | ✓ | 最上層に表示される文字と位置を交換可能 |
| {{ICON_LOCK}} ロック文字 | ✓ | ✓ | 編集文字層または参照文字層から ⌘ + ドラッグで追加可能 |

> [!TIP]
> 編集中の文字を特定の位置に固定するには、**⌘ + ドラッグ**で中央セルからターゲット位置にドラッグすることでロック文字を作成できます。
...
| {{INFO_MENU_TESTIMONIAL|}} | 使用体験をソーシャルメディアで共有 |
